...
1. **股票搜索/下拉**：全 A 股 5549+ 条（本地 JSON + 拼音首字母），输入式 datalist 模糊搜索（代码/名称/拼音，如 "zwgs"→中微公司）
...
| 功能 | 说明 |
|---|---|
| 股票搜索/下拉 | 全 A 股 5549+（本地 JSON + 拼音首字母），输入式 datalist（代码/名称/拼音模糊），★ 标记自选 |
...
| `GET /api/search?q=` | 股票搜索（全 A 股本地 + 拼音） |
| `GET /api/announcement?code=ANxxx` | 公告正文（标题/日期/正文/PDF 链接） |
| `GET /api/many?codes=a,b,c` | 批量报价（自选表格用） |
...
| Sprint 3 | 自选批量表格 + 分钟资金流明细 + 全A股拼音搜索 | ✅ 2026-08-14 |
| Sprint 4 | 多日主力对比 + 异动告警 + CSV导出（复盘记录 backlog） | ✅ 2026-08-14（US-017 ⏳） |

## 发布历史

| 版本 | 日期 | 说明 |
|---|---|---|
| V0.0.1 | 2026-08-13 | Sprint 1~2（MVP + 搜索 + K线 + 通达信本地化） |
| V0.0.2 | 2026-08-14 | Sprint 3~4（自选表格/资金流明细/5日主力/异动/CSV/右键副图/全A股搜索） |
